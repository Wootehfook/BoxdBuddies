...
- AI attribution header format (required): Include on the first line of every AI-generated file. Format: `// AI Generated: <model-name> - YYYY-MM-DD` for code files, and `<!-- AI Generated: <model-name> - YYYY-MM-DD -->` for Markdown. Tag with the model name only — do not prefix with a tool/product name (e.g. "GitHub Copilot"), since the same model can be invoked from different tools (Copilot Chat, Claude Code, etc.) and the tool name adds no provenance value. For new files or major rewrites, use the actual model name that generated the code (e.g., `Claude Haiku 4.5`, `Claude Sonnet 4.6`). When editing existing files that already have an older attribution (including the legacy `GitHub Copilot (<model-name>)` form), you may keep it for consistency, or update to the actual model name if doing a significant rewrite. Do not use generic placeholders like `<actual-model-name>` in committed code.

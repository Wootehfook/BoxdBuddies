...
/**
 * Simple smoke tests for BoxdBuddies application
 * AI Generated: GitHub Copilot - 2025-08-02
 */

test.describe('BoxdBuddies Smoke Tests', () => {
  test('should load the application successfully', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Letterboxd Username' })).toBeVisible();
    await expect(page.getByRole('textbox', { name: 'TMDB API Key (Optional)' })).toBeVisible();
  ...
  test('should accept valid input format', async ({ page }) => {
    ...
    const apiKeyInput = page.getByRole('textbox', { name: 'TMDB API Key (Optional)' });
    ...
    console.log('✅ Form inputs accepting data correctly');
  });
});

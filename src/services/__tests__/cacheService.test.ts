...
describe('Cache Service', () => {
  ...
  it('should handle year parsing edge cases', () => {
    ...
    yearTestCases.forEach(({ input, expected }) => {
      ...
      const validated = parsed && parsed > 1800 && parsed <= new Date().getFullYear() + 5 ? parsed : null;
  ...
  it('should validate TMDB API response structure', () => {
    ...
    const directors = mockTmdbResponse.credits.crew.filter(person => person.job === 'Director');

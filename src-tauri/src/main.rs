...
async fn save_user_preferences(request: SaveUserPreferencesRequest) -> Result<(), String> {
    ...
    log_debug(&format!(
        "🔧 SAVE_USER_PREFERENCES: Preferences path: {:?}",
        preferences_path
    ));
    ...
    log_debug(&format!(
        "🔧 SAVE_USER_PREFERENCES: Serialized preferences: {}",
        json_data
    ));
    ...
    log_debug(&format!(
        "🔧 SAVE_USER_PREFERENCES: SUCCESS - Saved user preferences to: {:?}",
        preferences_path
    ));
    Ok(())
}
    ...
    for movie_result in watchlist_iter {
        ...
        match movie_result {
            ...
            Err(e) => {
                ...
                return Err(format!(
                    "Failed to process movie at row {}: {}",
                    row_count, e
                ));
            }
        }
...
async fn save_watchlist_to_cache(
    ...
    println!(
        "Ensuring friend '{}' exists in friends table...",
        friend_username
    );
    {
        let friend_tx = conn
...
async fn get_letterboxd_watchlist_count(username: &str) -> Result<usize, String> {
    println!(
        "🔥 COUNT CHECK: Getting current watchlist count for {} from Letterboxd",
        username
    );
    ...
    println!(
        "🔥 COUNT CHECK: Found {} actual watchlist movies on page 1",
        film_count
    );
    ...
    if film_count >= 25 {
        ...
        println!(
            "🔥 COUNT CHECK: Single page watchlist with {} movies",
            film_count
        );
        Ok(film_count)
    }
...
async fn is_watchlist_cache_fresh_with_count_check(
    ...
    match get_friend_sync_status(friend_username.clone()).await? {
        Some(status) => {
            ...
            if let Some(last_sync) = status.last_watchlist_sync {
                ...
                if age_hours >= max_age_hours as i64 {
                    println!(
                        "🔥 CACHE CHECK: Cache too old ({} hours), not fresh",
                        age_hours
                    );
                    return Ok(false);
                }
                ...
                match get_letterboxd_watchlist_count(&friend_username).await {
                    Ok(current_count) => {
                        println!(
                            "🔥 CACHE CHECK: Current Letterboxd count: {}",
                            current_count
                        );
                        let counts_match = cached_count == current_count;
                        println!(
                            "🔥 CACHE CHECK: Cache age: {} hours, counts match: {}, is_fresh: {}",
                            age_hours, counts_match, counts_match
                        );
                        Ok(counts_match)
                    }
                    Err(e) => {
        ...
        None => {
            println!(
                "🔥 CACHE CHECK: No sync status found for user {}",
                friend_username
            );
            Ok(false)
        }
...
async fn is_watchlist_cache_fresh(
    ...
    println!(
        "🔥 CACHE CHECK: Checking cache freshness for {} (max age: {} hours)",
        friend_username, max_age_hours
    );
    match get_friend_sync_status(friend_username.clone()).await? {
        Some(status) => {
            println!(
            ...
            if let Some(last_sync) = status.last_watchlist_sync {
                ...
                println!(
                    "🔥 CACHE CHECK: Cache age: {} hours, is_fresh: {}",
                    age_hours, is_fresh
                );
                Ok(is_fresh)
            } else {
...
async fn get_watchlist_cached_or_scrape(
    ...
    match scrape_user_watchlist(&friend_username).await {
        ...
        Err(e) => {
            ...
            println!(
                "Scraping failed for {}, trying cached data: {}",
                friend_username, e
            );
            let cached_movies = get_cached_watchlist(friend_username).await?;
...
async fn scrape_letterboxd_friends(username: String) -> Result<Vec<LetterboxdFriend>, String> {
    ...
    loop {
        ...
        let url = if page == 1 {
            ...
            format!(
                "https://letterboxd.com/{}/following/page/{}/",
                username, page
            )
        };
...
async fn compare_watchlists(
    ...
    println!(
        "Comparing watchlists for {} with friends: {:?}",
        main_username, friend_usernames
    );
    ...
    if filtered_friends.len() != friend_usernames.len() {
        println!(
            "DEBUG: Filtered out main user '{}' from friends list",
            main_username
        );
    ...
    if !main_watchlist.is_empty() {
        println!(
            "🔍 DEBUG: First 5 movies from main user ({}):",
            main_username
        );
        ...
        if let Some(slug) = element.value().attr("data-film-slug") {
            ...
            if let Some(img) = element.select(&img_selector).next() {
                if let Some(alt_text) = img.value().attr("alt") {
                    ...
                    if !title.is_empty() {
                        println!(
                            "DEBUG: Extracted movie - Title: '{}', Year: {:?}",
                            title, year
                        );
            ...
            if let Some(target_link) = element.value().attr("data-target-link") {
                ...
                if let Some(img) = element.select(&img_selector).next() {
                    if let Some(alt_text) = img.value().attr("alt") {
                        ...
                        if !title.is_empty() {
                            println!(
                                "DEBUG: Extracted movie via target-link - Title: '{}', Year: {:?}",
                                title, year
                            );
                            movies.push(WatchlistMovie {
                                title,
...
async fn search_tmdb_movie(
    ...
    println!(
        "🎬 TMDB SEARCH: Searching for '{}' (year: {:?})",
        title, year
    );
...
async fn get_tmdb_movie_credits(api_key: &str, tmdb_id: i32) -> Result<Option<String>, String> {
    ...
    let url = format!("https://api.themoviedb.org/3/movie/{}/credits", tmdb_id);
...
fn get_cached_tmdb_movie_sync(
    ...
    let movie_iter = stmt.query_map([tmdb_id], |row| {
        Ok(TmdbMovieCache {
            tmdb_id: row.get(0)?,
            title: row.get(1)?,
            original_title: row.get(2)?,
...
fn create_enhanced_movie_from_details(
    ...
    let year = details
        ...
        .unwrap_or(movie.year.unwrap_or(0) as u32);
...
async fn enhance_movies_with_tmdb(
    ...
    println!("🎬 TMDB: Enhancing {} movies with TMDB data", total_movies);
...
async fn get_watchlist_size(username: &str) -> Result<usize, String> {
    ...
    println!(
        "DEBUG: get_watchlist_size for {}: found {} movies on first page",
        username, movies_on_page
    );
    ...
    if movies_on_page >= 28 {
        ...
        if has_next_page_in_html(&html_content) {
            ...
            println!(
                "DEBUG: get_watchlist_size for {}: detected max page {}",
                username, max_page
            );
            ...
            if max_page > 1 {
                ...
                println!(
                    "DEBUG: get_watchlist_size for {}: estimated total {}",
                    username, estimated_total
                );
                ...
                println!(
                    "DEBUG: get_watchlist_size for {}: estimated total (no pagination) {}",
                    username, estimated_total
                );
                return Ok(estimated_total);
            }
    ...
    println!(
        "DEBUG: get_watchlist_size for {}: small watchlist, actual count {}",
        username, movies_on_page
    );
    Ok(movies_on_page)
}
...
async fn get_database_info() -> Result<String, String> {
    let db_path = get_database_path()?;
    let mut info = format!("Database path: {:?}\n", db_path);
    ...
    match Connection::open(&db_path) {
        Ok(conn) => {
            ...
            if let Ok(count) = conn.query_row("SELECT COUNT(*) FROM friend_watchlists", [], |row| {
                ...
                info.push_str(&format!("🎬 Movies in cache: {}\n", count));
            }
            ...
            if let Ok(count) = conn.query_row("SELECT COUNT(*) FROM friends", [], |row| {
                ...
                info.push_str(&format!("👥 Friends in database: {}\n", count));
            }
            ...
            if let Ok(count) = conn.query_row(
                ...
                info.push_str(&format!("🖼️ Movies with posters: {}\n", count));
            }
        }
        Err(e) => {
            info.push_str(&format!("❌ Failed to connect to database: {}\n", e));
        }
    }
...
async fn clear_movie_cache(movie_title: String) -> Result<String, String> {
    ...
    let watchlist_updated = conn
        .execute(
            ...
            &[&movie_title as &dyn rusqlite::ToSql],
        )
        .map_err(|e| format!("Failed to clear cached TMDB data: {}", e))?;

    println!(
        "🗑️ CLEAR CACHE: Updated {} rows in friend_watchlists for '{}'",
        watchlist_updated, movie_title
    );

    Ok(format!(
        "Cleared cache for movie: {} (updated {} watchlist entries)",
        movie_title, watchlist_updated
    ))
}
